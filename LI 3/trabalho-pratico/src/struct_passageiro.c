...
void inserir_passageiro (char* id_user,GHashTable* hash_id_users,voo** hash_voos,int id_novo_voo)
{
        ...
        if (strcmp (g_hash_table_lookup(hash_id_users,id_user),INATIVO) == 0) return;
        ...
        if(*arvore == NULL)
        {
            ...
            int data[6];
            departure_date (hash_voos[id_novo_voo -1],data);
